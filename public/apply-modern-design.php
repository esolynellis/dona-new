...
// Remove dona-modern CSS from head_code
$check = $pdo->query("SELECT id, value FROM settings WHERE space='base' AND name='head_code' LIMIT 1")->fetch(PDO::FETCH_ASSOC);
if ($check) {
    $cleaned = preg_replace('/<link[^>]+dona-modern[^>]+>\n?/', '', $check['value']);
    $pdo->prepare("UPDATE settings SET value=? WHERE id=?")->execute([trim($cleaned), $check['id']]);
}
...
echo json_encode(['done' => true, 'reverted' => true], JSON_PRETTY_PRINT);
