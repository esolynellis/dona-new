...
{{--              <button class="btn btn-outline-danger btn-sm ml-1" type="button" @click="deleteItem(brand.brand_id, index)">{{ __('common.delete') }}</button>--}}
{{--            </td>--}}
          </tr>
          </tbody>
    ...
    <el-dialog title="{{ __('admin/common.brand') }}" :visible.sync="dialog.show" width="600px"
      ...
      <el-form ref="form" :rules="rules" :model="dialog.form" label-width="100px">
        ...
        <el-form-item label="{{ __('brand.name') }}" prop="name">
          <el-input class="mb-0" v-model="dialog.form.brand_name" placeholder="{{ __('brand.name') }}"></el-input>
        </el-form-item>
...
{{--        </el-form-item>--}}
        ...
        <el-form-item label="{{ __('common.sort_order') }}">
          <el-input class="mb-0" type="number" v-model="dialog.form.sort" placeholder="{{ __('common.sort_order') }}"></el-input>
        </el-form-item>
  ...
  <script>
    new Vue({
      ...
      data: {
        ...
        rules: {
          brand_name: [{required: true,message: '{{ __('common.error_required', ['name' => __('common.name')])}}',trigger: 'blur'}, ],
      ...
      computed: {
        url() {
          const url = @json(admin_route('huipi_brands.index'));
      ...
      methods: {
        loadData() {
          ...
          $http.get(`huipi_brand?page=${this.page}`).then((res) => {
        ...
        submit(form) {
          ...
          const url = this.dialog.type == 'add' ? 'huipi_brand' : 'huipi_brand/' + this.dialog.form.brand_id;
        ...
        deleteItem(id, index) {
          ...
          this.$confirm('{{ __('common.confirm_delete') }}', '{{ __('common.text_hint') }}', {
            ...
            $http.delete('huipi_brand/' + id).then((res) => {
              this.$message.success(res.message);
