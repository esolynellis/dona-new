...
class Sidebar extends Component
{
    ...
    private function getMenus(): mixed
    {
        $menus = [
            ...
            [
                'route'    => 'huipi_brands.index',
                'title'    => trans('admin/common.huipi_index'),
                'icon'     => 'bi bi-box-seam',
                'prefixes' => $this->getHuipiSubPrefix(),
                'children' => $this->getHuipiSubRoutes(),
            ],
            [
                'route'    => 'help.index',
     ...
     * 获取后台商品同步子页面路由前缀列表
     */
    private function getHuipiSubPrefix()
    {
        $prefix = ['huipi_sites','huipi_brands','huipi_goods','huipi_categorys'];

        return hook_filter('admin.sidebar.huipi.prefix', $prefix);
    }
    /**
     * 获取后台产品子页面路由前缀列表
     */
     ...
     * 获取商品子页面路由
     */
    public function getHuipiSubRoutes()
    {
        $routes = [
            ['route' => 'huipi_sites.index', 'prefixes' => ['huipi_sites']],
            ['route' => 'huipi_brands.index', 'prefixes' => ['huipi_brands']],
            ['route' => 'huipi_goods.index', 'prefixes' => ['huipi_goods']],
            ['route' => 'huipi_categorys.index', 'prefixes' => ['huipi_categorys']],
        ];

        return hook_filter('admin.sidebar.huipi_routes', $routes);
    }
