...
class ProductRepo
{
    ...
    {
        $builder = static::getBuilderWyl(array_merge(['category_id' => $categoryId, 'active' => 1], $filterData));
    ...
    public static function getBuilder(array $filters = []): Builder
    {
        ...
        $builder->whereNotIn('products.id', [1,2]);
     ...
     * @throws \Exception
     */
    public static function getBuilderNew(array $filters = []): Builder
    {
        $builder = Product::query()->with(['description', 'skus', 'masterSku', 'attributes', 'brand']);
        ...
        if (isset($filters['category_id'])) {
            $builder->whereHas('categories', function ($query) use ($filters) {
                if (is_array($filters['category_id'])) {
                    ...
                    //查询分类详情
                    $cateinfo = Category::find($filters['category_id']);
                    if($cateinfo['parent_id']!==0){
                        $categories = Category::query()->where('parent_id', $filters['category_id'])->pluck('id');
...
//                      $query->where('category_id', $filters['category_id']);
    ...
    public static function getBuilderWyl(array $filters = []): Builder
    {
        $builder = Product::query()->with(['description', 'skus', 'masterSku', 'attributes', 'brand']);
        ...
        $builder->select(['products.*', 'pd.name', 'pd.content', 'pd.meta_title', 'pd.meta_description', 'pd.meta_keywords']);
...
//            });
            ...
            $builder->whereHas('categories', function ($query) use ($filters) {
                if (is_array($filters['category_id'])) {
                    ...
                    $categories = Category::query()->where('parent_id', $filters['category_id'])->pluck('id');
                    $categories = $categories->toArray();
                    $categories1 = Category::query()->whereIn('parent_id', $categories)->pluck('id');
                    $categories1 = $categories1->toArray();
                    $ids = array_merge($categories,$categories1);
                    $ids[]= $filters['category_id'];
        ...
        if ($keyword) {
            ...
            $builder->where(function (Builder $query) use ($keywords) {
                ...
                foreach ($keywords as $keyword) {
                    $query->orWhere('pd.name', 'like', "%{$keyword}%");
                    $query->orWhere('goods_code2', 'like', "%{$keyword}%");
                }
            });
        }
        ...
        elseif ($sort == 'views') {
            $sort = 'views_count';
            // 左连接浏览记录表并计算每个产品的浏览总数
            $builder->leftJoin('product_views', 'product_views.product_id', '=', 'products.id')
        ...
        // var_dump($builder->toSql());die;
        return hook_filter('repo.product.builder', $builder);
    }
    public static function getBuilderOld(array $filters = []): Builder
    {
        $builder = Product::query()->with(['description', 'skus', 'masterSku', 'attributes', 'brand']);
    ...
    public static function fenxiaogoods(){
        $goods = Product::query()->whereIn('id',[1,2])->with(['description', 'skus', 'masterSku', 'attributes', 'brand'])->get();
    ...
    public static function getCateName($product_id){
        $cateinfo = ProductCategory::query()->where('product_id',$product_id)->first();
        $catedesinfo = CategoryDescription::query()->where('locale',locale())->where('category_id',$cateinfo['category_id'])->first();
        return $catedesinfo;
    }
}
