...
class HuipiBrandRepo
{
    ...
    public static function create($data)
    {
        ...
        return HuipiBrand::query()->create($brandData);
    }
    ...
    public static function update($brand, $data)
    {
        if (! $brand instanceof Brand) {
            $brand = HuipiBrand::query()->find($brand);
        }
        if (! $brand) {
    ...
    {
        return HuipiBrand::query()->find($id);
    }
    ...
    {
        if (! $brand instanceof Brand) {
            $brand = HuipiBrand::query()->find((int) $brand);
        }
    ...
    {
        $builder = HuipiBrand::query();
        if (isset($filters['brand_name'])) {
            $builder->where('brand_name', 'like', "%{$filters['brand_name']}%");
        }
...
//        var_dump($sql);die;
    ...
    {
        $brands = HuipiBrand::query()->where('status', true)->get();
    ...
    public static function getListByIds($ids)
    {
        ...
        $builder = HuipiBrand::query()->whereIn('id', $ids);
        $ids     = implode(',', $ids);
