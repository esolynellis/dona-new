...
class ProductController extends Controller
{
    ...
    public function show(Request $request, Product $product)
    {
        ...
        if($data['product']['gunit_max']){
            $gunit_text .= '1'.$data['product']['gunit_max'];
        }
        if($data['product']['gnum_midd']>0){
            $gunit_text .= '*'.$data['product']['gnum_midd'].$data['product']['gunit_midd'];
        }
        if($data['product']['gunit_min']>0){
            $gunit_text .= '*'.$data['product']['gnum_min'].$data['product']['gunit_min'];
        }
        $data['product']['gunit_text'] = $gunit_text;
