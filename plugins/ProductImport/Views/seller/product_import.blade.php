...
<div class="card mb-3">
  ...
  <div class="card-body">
    <form id="form-export" class="form-horizontal no-load" method="post" action="{{ route("seller.import.export") }}">
      @csrf
  ...
  <script>
    ...
    $('#button-import').on('click', function() {
      ...
      layer.confirm('{{ trans('ProductImport::common.text_upload_confirm') }}', {
        ...
        $.ajax({
          url: '{{ route('seller.import.upload') }}',
          type: 'post',
          data: formData,
          processData: false,
          contentType: false,
    ...
    function job() {
      ...
      $.ajax({
        url: '{{ route('seller.import.import') }}?index=' + index,
        type: 'post',
        contentType: false,
        dataType: 'json',
        success: function(json) {
