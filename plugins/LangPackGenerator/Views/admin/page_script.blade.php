  ...
  async function translatePage(obj, isEditoCallback=false) {
    ...
    for (let f = 0; f < fields.length; f++) {
      ...
      var tabId = $parents.find('div[role="tabpanel"].active').map(function() {
        // 使用正则表达式匹配 tab- 后面的文本
        var match = this.getAttribute('id').match(/tab-(.*)/);
        ...
        if(res.status === "fail"){
          ...
          if (isError) {
            ...
            res.data.forEach((e) => {
              ...
              $parents.find('button[data-bs-target="#tab-'+e.locale+'"]').parent().click();
              // console.log($parents.parent().find('button[data-bs-target="#tab-'+e.locale+'"]').html(), "dddddd")
              window.parent.postMessage({ type: 'UPDATE_TINYMCE', content: e.result, tinyID:tinyID }, '*');
              obj.val(e.result);
              objTextarea.val(e.result);
