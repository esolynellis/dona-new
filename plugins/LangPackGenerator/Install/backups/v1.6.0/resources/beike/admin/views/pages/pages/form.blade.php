...
@section('page-bottom-btns')
  <button type="button" class="w-min-100 btn btn-primary submit-form btn-lg"
          form="form-page">{{ __('common.save') }}</button>
...
@section('content')
  ...
  <div id="app" class="card h-min-600">
    <div class="card-body">
      ...
      @endif
      <form novalidate id="form-page" class="needs-validation"
            action="{{ $page->id ? admin_route('pages.update', [$page->id]) : admin_route('pages.store') }}"
            method="POST">
        <div class="tab-content">
                  ...
                  <x-admin::form.row title="{{ __('page_category.text_summary') }}">
                    <div class="input-group w-max-400">
                      ...
                      <textarea rows="3" type="text" name="descriptions[{{ $language['code'] }}][summary]"
                                class="form-control wp-400 {{ $is_invalid }}"
          ...
          <div class="tab-pane fade" id="tab-data">
            ...
            <x-admin-form-image name="image" title="{{ __('admin/page.cover_picture') }}"
                                value="{{ old('image', $page->image ?? '') }}">
              <div class="help-text font-size-12 lh-base">{{ __('common.recommend_size') }}: 500*350</div>
            </x-admin-form-image>

            <x-admin-form-input name="views" title="{{ __('page_category.views') }}"
                                value="{{ old('views', $page->views ?? '') }}"/>
            ...
            <x-admin-form-switch name="active" title="{{ __('common.status') }}"
                                 value="{{ old('active', $page->active ?? 1) }}"/>
          </div>
        </div>
