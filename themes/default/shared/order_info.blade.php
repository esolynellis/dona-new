  ...
  <div class="card-header d-flex align-items-center justify-content-between">
    ...
    <div>
      @if ($order->status == 'unpaid')
        @if ($order->payment_method_name == 'Qpay')
          <button class="btn btn-primary btn-sm nowrap qpay-button" type="button">{{ __('shop/account/order_info.to_pay') }}</button>
        @else
...
@push('add-scripts')
  <script>
    ...
    // 添加Qpay支付按钮的点击事件
    $('.qpay-button').click(function() {
      const btn = $(this);
      btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Processing...');

      $http.post('orders/{{ $order->number }}/qpay', {
        order_sn: '{{ $order->number }}'
      }).then((res) => {
        if(res.qPay_shortUrl) {
          // 显示二维码和支付选项弹窗
          showQpayModal(res);
        } else {
          layer.msg(res.message || 'Payment initialization failed');
          btn.prop('disabled', false).html('{{ __('shop/account/order_info.to_pay') }}');
        }
      }).catch((error) => {
        layer.msg(error.message || 'Payment request failed');
        btn.prop('disabled', false).html('{{ __('shop/account/order_info.to_pay') }}');
      });
    });
    // 显示Qpay支付弹窗 - 简化版
    function showQpayModal(response) {
      const modalHtml = `
  <div style="padding: 20px; text-align: center;">
    <h4 style="margin-bottom: 20px;">QPay</h4>
    <img
      src="data:image/png;base64,${response.qr_image}"
      alt="Qpay QR Code"
      style="max-width: 250px; border: 1px solid #ddd; padding: 15px; min-height: 250px;"
      onload="layer.iframeAuto(layer.index)"
    >
  </div>
`;
